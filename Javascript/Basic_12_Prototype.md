...
모든 객체는 자신의 프로토타입 객체를 가리키는 [[Prototype]] 인터널 슬롯(internal slot)을 가지며 상속을 위해 사용한다.
...
하지만 String 객체의 프로토타입 객체 String.prototype에 메서드를 추가하면 원시 타입, 객체 모두 메서드를 사용할 수 있다.
